/** 勿扰时段设置 API — 薄 re-export 层（实现统一收敛在 src/lib/api.ts）。
 *  复用既有命令 commands::settings_cmd 的 get_all_settings / set_setting。
 *  启用勿扰时两个键各写 "HH:MM"，关闭时写空串
 *  （Rust 侧读取端容错：缺失/空 = 始终通知，见 src-tauri/src/dnd.rs）。 */
export { getAllSettings, setSetting } from "@/lib/api";
